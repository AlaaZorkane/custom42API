...
const config = {
	promo: "2019-03",
	URL: "/v2/cursus/1/cursus_users?filter[campus_id]=16&filter[begin_at]=2019-03-24T23:00:00.000Z",
	// URL: "/v2/cursus/21/cursus_users?filter[campus_id]=16"
};
function msleep(n) {
...
(async () => {
	...
	const gibUsers = async (page) => new Promise(async (res, rej) => {
		try {
			const users = (await got(`${config.URL}&page[number]=${page}&page[size]=100`, {
				baseUrl,
				headers: {
